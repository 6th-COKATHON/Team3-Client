const generateRandomPolygon = (width: number, height: number): string[] => {
  ...
  const picTL = `polygon(${leftPoint}, ${TL}, ${topPoint}, ${centerPoint})`;
  const picTR = `polygon(${topPoint}, ${TR}, ${rightPoint}, ${centerPoint})`;
  const picBR = `polygon(${rightPoint}, ${BR},${bottomPoint}, ${centerPoint})`;
  const picBL = `polygon(${bottomPoint}, ${BL}, ${leftPoint}, ${centerPoint})`;
